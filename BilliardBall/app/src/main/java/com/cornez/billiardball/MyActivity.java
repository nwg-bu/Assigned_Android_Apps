...
public class MyActivity extends Activity implements
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        seekBar = (SeekBar)findViewById(R.id.sbAccelerate);
        seekBar.setProgress(0);
        seekBar.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            @Override
            public void onProgressChanged(SeekBar seekBar, int progress, boolean fromUser) {
                if(progress == 0){
                    mBall.setFRICTION(0.93);
                }else if(progress ==1){
                    mBall.setFRICTION(progress);
                }else{
                    mBall.setFRICTION(1.05);
                }
                Log.i("seek","progress"+progress);
            }
    ...
    private void buildBall(int windowWidth, int windowHeight) {
        ...
        mBall.setRadius(120);
        mBall.setCollisionBoundaries(-windowWidth / 2, windowWidth / 2, -windowHeight / 2, windowHeight / 2);
